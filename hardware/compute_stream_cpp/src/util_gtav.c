...
void save_png_texture(char *file_name, unsigned char *data, unsigned int width, unsigned int height, int is_depth) {
    unsigned int error;
    error = lodepng_encode_file(file_name, data, width, height, (is_depth) ? LCT_GREY : LCT_RGB, (is_depth) ? 16 : 8);
    if (error) {
        fprintf(stderr, "save_png_texture %s failed, error %u: %s\n", file_name, error, lodepng_error_text(error));
        exit(1);
    }
}
